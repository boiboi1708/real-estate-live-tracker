real-estate-live-tracker/
├── .github/                 # GitHub Actions (CI/CD sau này)
├── docs/                    # Tài liệu kế hoạch, thiết kế
├── client/                  # Frontend (React/Next.js)
│   ├── src/
│   │   ├── components/      # UI chung
│   │   ├── modules/         # <--- CHIA MODULE TẠI ĐÂY
│   │   │   ├── auth/        # Login (Visitor/Agency logic)
│   │   │   ├── map/         # Map view, Markers
│   │   │   ├── agency/      # Dashboard, Inventory
│   │   │   └── visitor/     # Wishlist, Search
├── server/                  # Backend (Node.js/NestJS)
│   ├── src/
│   │   ├── modules/         # Đồng bộ module với frontend
│   │   │   ├── auth/
│   │   │   ├── real-time/   # Socket Gateway
│   │   │   └── properties/
├── README.md                # Giới thiệu dự án & Hướng dẫn chạy
└── docker-compose.yml       # Để chạy toàn bộ database/app bằng 1 lệnh

Milestone 1: The Walking Skeleton (Khung sườn) - Ưu tiên cao nhất
Mục tiêu: Chạy được ứng dụng, có trang Login và trang Dashboard rỗng.

Task 1: Init Project (Setup Client & Server).

Task 2: Tạo UI trang Login (Auth Module).

Yêu cầu: Default tab là "Visitor". Switch qua lại "Agency".

Task 3: Setup Navigation (Routing).

Milestone 2: Map & Inventory Core
Mục tiêu: Agency tạo được 1 căn nhà, Visitor thấy được căn đó trên bản đồ (tĩnh).

Task 4: Database Schema setup.

Task 5: Agency CRUD (Tạo/Sửa nhà).

Task 6: Tích hợp Google Maps/Mapbox hiển thị markers.

Milestone 3: Real-time Activation
Mục tiêu: Agency bấm nút -> Visitor thấy thay đổi ngay lập tức.

Task 7: Setup Socket.io.

Task 8: Kết nối sự kiện "Go Live" từ Agency tới Map của Visitor.

3. Quy trình làm việc (Git Workflow)
Để code không bị loạn, bạn nên tuân thủ quy tắc sau ngay từ đầu:

Nhánh main: Chỉ chứa code đã chạy ổn định (Production ready).

Nhánh develop: Code đang phát triển tích hợp.

Nhánh tính năng (feature/login-page, feature/map-view): Mỗi khi làm một module mới, hãy tách ra một nhánh riêng từ develop.



I. Kiến trúc & Phân chia Modules (Modular Architecture)
Để đảm bảo khả năng mở rộng và bảo trì, hệ thống sẽ được chia thành các modules độc lập nhưng liên kết chặt chẽ với nhau.
